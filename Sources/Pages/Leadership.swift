...
struct Leadership: StaticPage {
	...
	func body(context: PublishingContext) -> [BlockElement] {
		Table {
			if let officers = context.decode(resource: "leadership.json", as: [CommitteeMember].self)?.sorted(by: { $0.priority < $1.priority && $0.lastName < $1.lastName }) {
				for members in officers.batched(into: 4) {
					Row {
