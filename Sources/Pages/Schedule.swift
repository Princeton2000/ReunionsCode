...
struct Schedule: StaticPage {
	...
	//	var events: [Event] { let decoder = JSONDecoder(); let data = try! Data(contentsOf: URL(fileURLWithPath: "/Users/jpurnell/Dropbox/Computer/Development/Swift/Princeton/Website/ExampleSite/Resources/events_real.json"));
		var events: [Event] { let decoder = JSONDecoder(); let data = try! Data(contentsOf: URL(fileURLWithPath: "/Users/jpurnell/Dropbox/Computer/Development/Swift/Princeton/Website/ExampleSite/Resources/events.json")); let events = try! decoder.decode([Event].self, from: data);  print(events.map({$0.summary}).joined(separator: "\n")); return events.sorted(by: {$0.startDate < $1.startDate})}
