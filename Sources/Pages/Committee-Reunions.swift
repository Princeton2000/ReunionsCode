...
struct CommitteeReunions: StaticPage {
	...
	func body(context: PublishingContext) -> [BlockElement] {	
		Table {
			if let committee = context.decode(resource: "leadership.json", as: [CommitteeMember].self)?.filter({$0.priority < 3 && $0.priority > 0}).sorted(by: {$0.priority < $1.priority && $0.role < $1.role && $0.lastName < $1.lastName && $0.firstName < $1.firstName}) {
